...
public final class EventStateHandler {
	...
	public static void moveToDraft(Event event) {
		if (event.getState() != Event.EventState.REVIEW && event.getState() != Event.EventState.PUBLISHED) {
			throw new IllegalStateException("Can only move to draft from REVIEW or PUBLISHED state");
		}
